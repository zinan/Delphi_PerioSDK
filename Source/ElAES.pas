...
type
  ...
  TByteBuffer = array [0 .. 512 - 1] of byte;
...
procedure DecryptAESStreamCBC(Source: TStream; Count: cardinal;
  const ExpandedKey: TAESExpandedKey256;  const InitVector: TAESBuffer;
  Dest: TStream); overload;
function Encrypt (DataBuffer:TByteBuffer ;count:integer; var EncDataBuffer:TByteBuffer) : integer;
Function Decrypt (EncDataBuffer:TByteBuffer ;count:integer; var DataBuffer:TByteBuffer) : integer;
...
begin
  ExpandAESKeyForEncryption(Key, ExpandedKey);
  EncryptAESStreamECB(Source, 0, ExpandedKey, Dest);
end;


procedure EncryptAESStream(DataBuffer:TByteBuffer;
  const ExpandedKey: TAESExpandedKey128; datacount:integer;var EncDataBuffer:TByteBuffer);
var
...
begin
  ...
  begin
       Move( DataBuffer[i*16], TempIn, Length( TempIn ));
     //   strlcopy(  ,PChar(DataBuffer), SizeOf(TempIn));
  //  Done := MOVE(PChar(TempIn),PChar(DataBuffer), SizeOf(TempIn));
  //  Done:=Length( TempIn );
   // if Done < SizeOf(TempIn) then
    //  raise EStreamError.Create(SReadError);
    EncryptAES(TempIn, ExpandedKey, TempOut);
    Move( TempOut, EncDataBuffer[i*16], Length( TempOut ));
   // EncDataBuffer := EncDataBuffer+TempOut;
...
//procedure Encrypt(Source: TStream; Count: cardinal;
//  const Key: TAESKey128; Dest: TStream);

procedure DecryptAESStream(EncDataBufferr:TByteBuffer;
  const ExpandedKey: TAESExpandedKey128; datacount:integer;var DataBuffer:TByteBuffer);
var
  TempIn, TempOut: TAESBuffer;
  Done: cardinal;
  Count,i:integer ;
begin
  ...
  begin
   ...
   Move( EncDataBufferr[i*16], TempIn, Length( TempIn ));

    DecryptAES(TempIn, ExpandedKey, TempOut);
    Move( TempOut, DataBuffer[i*16], Length( TempOut ));
    Dec(Count, SizeOf(TAESBuffer));
    i:=i+1;
  end;
...
FillChar( Key, SizeOf(Key), 48 );
   FillChar( EncDataBuffer, SizeOf(DataBuffer), 0 );
Move(PChar('0000000000000000')^, Key, Min(SizeOf('0000000000000000'), Length('0000000000000000')));
    //Move( PChar(Edit2.Text)^, Key, Min( SizeOf( Key ), Length( Edit2.Text )));
  ExpandAESKeyForEncryption(Key, ExpandedKey);
 // EncryptAESStreamECB(Source, Count, ExpandedKey, Dest);
 EncryptAESStream(DataBuffer,ExpandedKey,count, EncDataBuffer);
 result:=1;
end;
...
Move(PChar('0000000000000000')^, Key, Min(SizeOf('0000000000000000'), Length('0000000000000000')));
 ...
 // EncryptAESStreamECB(Source, Count, ExpandedKey, Dest);
 DecryptAESStream(EncDataBuffer,ExpandedKey,count, DataBuffer);
 result:=1;
end;
